---
source_url: https://unsloth.ai/blog/gradient
slug: gradient
title: "Bug Fixes in LLM Training - Gradient Accumulation"
...
Bug Fixes in LLM Training - Gradient Accumulation   {"@context":"https://schema.org","@type":"WebSite","name":"Unsloth - Train and Run Models Locally","url":"https://unsloth.ai"}
...
## By Daniel & Michael
This past week, we've been fixing a universal [issue](https://github.com/huggingface/trl/issues/2175) in Gradient Accumulation that negatively impacts everyone's training, pre-training & finetuning runs for sequence models like LLMs. [Unsloth's](https://github.com/unslothai/unsloth) Gradient Accumulation fix ensures training runs and loss calculations are performed accurately and correctly.

The goal of gradient accumulation is to mimic full batch training with reduced VRAM usage. Gradient accumulation is also used in DDP and multi GPU setups, so this issue affects large scale training runs as well. P.S. If you’ve enjoyed our work, don't forget to [⭐Star us](https://github.com/unslothai/unsloth) on GitHub and join our awesome community on [Discord](https://discord.com/invite/unsloth) - your support means the world to us! 🦥

Previously in 2021, [Zhaofeng](https://github.com/huggingface/transformers/issues/14638) first discovered the issue, and [Benjamin Marie](https://x.com/bnjmn_marie) rediscovered the issue last week. They showed if you use gradient accumulation, you will get a higher loss than if you used full batch training:    We managed to formulate a new methodology that solves the issue - and it's now in Unsloth! Please update Unsloth (  pip install --upgrade unsloth  ) and use   unsloth_train  ! We have a free Colab notebook to finetune Llama 3.2 1/3B with our fixed trainer here - [Colab notebook](https://colab.research.google.com/drive/1z0XJU2FCzDC8oyXa2Nd4jCxylRMI-o0-?usp=sharing). And a free [Kaggle notebook](https://www.kaggle.com/code/danielhanchen/fixed-kaggle-llama-3-2-1b-3b-conversation/). `from unsloth import unsloth_train
# trainer_stats = trainer.train() << Buggy gradient accumulation
trainer_stats = unsloth_train(trainer)`  💡 Our Findings
...
During training or finetuning, one selects some number of random rows in the training dataset to update the model's weights at every step. But how many rows? For very large pretraining jobs, the batch size could be in the many millions like in [Llama 3.1](https://docs.unsloth.ai/get-started/all-our-models) to reduce overfitting and improve generalization capabilities. For finetuning jobs like in [Unsloth's Llama 3.2 notebook](https://colab.research.google.com/drive/1T5-zKWM_5OD21QHwXHiV9ixTRR7k3iB9?usp=sharing), the batch size could be a small 32.   The issue is the memory usage for large batches is quite large. If 1 batch uses 1 unit of memory, then a 1 million batch size would use 1 million units of memory. How do we mimic large batch training but also not use a lot of memory?
...
Are gradient accumulation and full batch training mathematically the same? Sadly not if you do it naively by simply adding them up! We first note that the [cross entropy loss](https://pytorch.org/docs/stable/generated/torch.nn.CrossEntropyLoss.html) is calculated by:
...
Another point to consider is maybe this error won't actually affect the difference in the final weights. So, we trained a LoRA adapter using [Unsloth](https://github.com/unslothai/unsloth) with the full batch (bsz=16, ga=1) vs just a gradient accumulated version (bsz=1, ga=16).
...
window.__remixContext = {"basename":"/","future":{"v3_fetcherPersist":false,"v3_relativeSplatPath":false,"v3_throwAbortReason":false,"v3_routeConfig":false,"v3_singleFetch":false,"v3_lazyRouteDiscovery":false,"unstable_optimizeDeps":false},"isSpaMode":false,"state":{"loaderData":{"root":null,"routes/[blog].[gradient]._index":{"host":"unsloth.ai","url":"https://unsloth.ai/blog/gradient","system":{"params":{},"search":{},"origin":"https://unsloth.ai","pathname":"/blog/gradient"},"resources":{},"pageMeta":{"title":"Bug Fixes in LLM Training - Gradient Accumulation","description":"Unsloth's Gradient Accumulation fix solves critical errors in LLM Training.","excludePageFromSearch":false,"socialImageAssetName":"L2_Norm_analysis_from_bsz16_ga1_L4dm8xQcPyLyrG_EaLEMz.svg","custom":[]}}},"actionData":null,"errors":null}};  import "/assets/manifest-64340927.js";
import * as route0 from "/assets/root-Bi8uDDMw.js";
import * as route1 from "/assets/_blog_._gradient_._index-LcKAI56S.js";

window.__remixRouteModules = {"root":route0,"routes/[blog].[gradient]._index":route1};
